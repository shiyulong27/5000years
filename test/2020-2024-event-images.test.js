...
describe('1970-2024 年 5 级（⭐⭐⭐⭐⭐ 历史转折）重大事件配图 100% 全覆盖强约束验证', () => {
  const events = [
  ...
  it('1970-2024 年共有 397 条 5 级重大事件', () => {
    expect(major5StarEvents.length).toBe(397)
  })

  it('所有 397 条 5 级重大事件必须 100% 配置图片、本地物理存在且具备非空图注', () => {
    for (const event of major5StarEvents) {
      expect(event.image, `${event.id} 缺少 image 属性`).toBeTypeOf('object')
      expect(event.image.url).toMatch(/^\/images\/events\/(?:19[789]\d|20\d{2})\/[a-zA-Z0-9_-]+\.(?:jpg|png|webp)$/)
